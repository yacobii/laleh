<x-layouts.app>
    <div class="flex flex-col size">
        <div class="mt-14 grow">
            ...
            <div class="">
                <div class="grid grid-cols-2 md:grid-cols-4 gap-5 md:gap-x-14 md:gap-y-20 mb-20 mt-5 mx-auto">
                    @foreach($products as $product)
                        <div class="relative w-full" wire:key="{{ $product->id }}">
                            <a href="{{ route('product', $product) }}">
                                <img class="w-full object-cover"
                                     src="{{ $product->variations?->where('type', 'color')->where('selected', true)->first()?->getFirstMediaUrl('colors', 'small') }}"
                                     title="{{ $product->name }}"
                                     alt="{{ $product->name }}">
                                <div class="py-3 space-y-2">
                                    <p class="text-right mt-2 font-bold h-14 text-sm md:text-base">{!! nl2br($product->name) !!}</p>
                                    <div class="text-gray-400 markdown text-sm h-20">
